...
class ReorgStakeTest(PivxTestFramework):
    ...
    def check_money_supply(self, expected_piv, expected_zpiv):
        ...
        # verify that nodes have the expected xczm and zxczm supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_piv))
            for denom in node['zxczmsupply']:
                assert_equal(node['zxczmsupply'][denom], DecimalAmt(expected_zpiv[denom]))
    ...
    def run_test(self):
        ...
        # Check xczm and zxczm supply at the beginning
        # ------------------------------------------
        # zxczm supply: 2 coins for each denomination
        ...
        # xczm supply: block rewards minus burned fees for minting
        ...
        # Verify that xczm and zxczm supplies were properly updated after the spends and reorgs
        self.log.info("Check xczm and zxczm supply...")
