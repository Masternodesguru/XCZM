...
CAmount CMasternode::GetBlockValue(int nHeight)
{
    ...
    if (nHeight >   1000000) return     0.1 * COIN;
    if (nHeight >    306510) return     1 * COIN;
    if (nHeight >    173800) return     1.5 * COIN;
    if (nHeight >    145000) return     2 * COIN;
    if (nHeight >    125200) return     5   * COIN;
    if (nHeight >     87400) return     1.5 * COIN;
    if (nHeight >     29880) return     2 * COIN;
    if (nHeight >     18280) return     2.5 * COIN;
    if (nHeight >         2) return     0.5 * COIN;
    if (nHeight >         1) return     8600000 * COIN;
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    if (mi != mapBlockIndex.end() && (*mi).second) {
        ...
        int nConfHeight = pMNIndex->nHeight + MASTERNODE_MIN_CONFIRMATIONS - 1; // block for 1000 xczm tx -> 1 confirmation
